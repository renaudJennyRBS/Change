...
/**
 * @name \Change\Workflow\Events\WorkflowResolver
 */
class WorkflowResolver
{
	...
	public function examine(\Zend\EventManager\Event $event)
	{
		...
		{
			$workflowModel = $documentServices->getModelManager()->getModelByName('Change_Workflow_Workflow');
			if ($workflowModel)
			{
	...
	public function process(\Zend\EventManager\Event $event)
	{
		...
		{
			$taskModel = $documentServices->getModelManager()->getModelByName('Change_Workflow_Task');
			if ($taskModel)
			{
				$task = $documentServices->getDocumentManager()->getDocumentInstance($taskId, $taskModel);
				if ($task instanceof \Change\Workflow\Documents\Task
					&& $task->getStatus() == \Change\Workflow\Interfaces\WorkItem::STATUS_ENABLED
				)
				{
					$workflowInstance = $task->getWorkflowInstance();
					if ($workflowInstance instanceof \Change\Workflow\Documents\WorkflowInstance
