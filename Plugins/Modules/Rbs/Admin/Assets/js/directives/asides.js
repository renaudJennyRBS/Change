(function($) {
	...
	app.directive('rbsDefaultAsidesForList', function() {
		return {
			restrict: 'E',
			template: '<rbs-aside-plugin-menu></rbs-aside-plugin-menu>' +
							...
							if (lcid === doc.refLCID) {
								contents.push({
									...
									'text': item.label + ' (<abbr title="' +
										i18n.trans('m.rbs.admin.adminjs.reference_language | ucf') + '">' +
										i18n.trans('m.rbs.admin.adminjs.ref_lang_abbr') + '</abbr>)',
									'icon': 'icon-book'
								});
							}
							else {
		...
		return {
			...
			link: function(scope) {
				...
				scope.seoCreate = function() {
					...
					REST.call(scope.document.getActionUrl('addSeo'), null,
							REST.resourceTransformer()).then(function(seoDocument) {
