...
(function() {
	 ...
	 */
	app.service('RbsChange.UserNotifications', ['RbsChange.REST', '$q', 'RbsChange.User', 'RbsChange.Settings', '$timeout',
		function(REST, $q, User, Settings, $timeout) {
			var notifications = {},
				allNotifications = {};
			...
			function getNotificationQuery(status) {
				var query = {
					...
					'where': {
						'and': [{
							'op': 'eq',
							'lexp': { 'property': 'userId' },
							'rexp': { 'value': User.get().id }
						}]
					},
					'order': [{
