(function () {
	...
	// /!\ Warning!
	// Do NOT "use strict" here: the OAuth stuff below doesn't work with "use strict" enabled.

	var app = angular.module('OAuthModule', ['LocalStorageModule']),
	    ...
	    cfgLocalStorageKeyName = 'OAuthData',
		cfgRealm = 'PLEASE_DEFINE_YOUR_REALM',
		...
		cfgInterceptJQueryAjaxCalls = true

		;


	/**
	 * Checks whether a URL needs to be signed or not.
	 *
	 * @param url
	 *
	 * @returns {boolean}
	...
	function shouldSignRequest (url) {
		...
		// Check if the request should be signed or not.
		if (angular.isString(cfgSignedUrlPatternInclude)) {
			shouldSign = url.indexOf(cfgSignedUrlPatternInclude) !== -1;
		} else if (cfgSignedUrlPatternInclude instanceof RegExp) {
			shouldSign = cfgSignedUrlPatternInclude.test(url);
		}

		// Check exclusions.
		if (shouldSign) {
			if (angular.isString(cfgSignedUrlPatternExclude)) {
				shouldSign = url.indexOf(cfgSignedUrlPatternExclude) === -1;
			} else if (cfgSignedUrlPatternExclude instanceof RegExp) {
				shouldSign = ! cfgSignedUrlPatternExclude.test(url);
			}
		}
	 ...
	 *
	 * This function comes from jsOAuth (https://github.com/bytespider/jsOAuth) but has been adapted (simplified)
	 * for our needs.
	 *
	 * This method is used by our Request Interceptor (see below) and the jQuery's AJAX hack (see below).
	 *
	 * @param config The AngularJS $http config object for the request to be signed.
	 *
	 * @returns {*} The config object for the request.
	 */
	function signRequest (config) {
		...
		// According to the OAuth spec, if data is transferred using multipart,
		// the POST data doesn't have to be signed:
		...
		signature = OAuth_signatureMethod[signatureMethod](oauthData.consumerSecret, oauthData.tokenSecret, signatureString);
		...
		config.headers['Authorization'] = 'OAuth ' + toHeaderString(headerParams);
	 ...
	 *
	 * This Service is configurable in the "module.config()" phase of AngularJS life cycle,
	 * before the Service instance is created.
	 */
	app.provider('OAuthService', function OAuthServiceProviderFn () {

		this.setBaseUrl = function (url) {
			cfgBaseUrl = url;
		};

		this.setRequestTokenUrl = function (url) {
			cfgRequestTokenUrl = url;
		};

		this.setRegisterUrl = function (url) {
			cfgRegisterUrl = url;
		};

		this.setAuthorizeUrl = function (url) {
			cfgAuthorizeUrl = url;
		};

		this.setAccessTokenUrl = function (url) {
			cfgAccessTokenUrl = url;
		};
		...
		this.setLocalStorageKeyName = function (name) {
			cfgLocalStorageKeyName = name;
		};

		this.setSignedUrlPatternInclude = function (pattern) {
			cfgSignedUrlPatternInclude = pattern;
		};

		this.setSignedUrlPatternExclude = function (pattern) {
			cfgSignedUrlPatternExclude = pattern;
		};

		this.interceptJQueryAjaxCalls = function (value) {
			cfgInterceptJQueryAjaxCalls = value;
		};


		/**
		 * Creates the OAuthService instance.
		 */
		this.$get = ['$http', '$rootScope', '$location', '$q', '$log', 'localStorageService', function ($http, $rootScope, $location, $q, $log, localStorageService) {

			// The full URL is required for the authentication process to work correctly.
			if ( ! cfgBaseUrl ) {

				throw new Error("You must configure the OAuthService to provide the base URL with setBaseUrl().");

			} else if (cfgBaseUrl.indexOf('http:') !== 0 && cfgBaseUrl.indexOf('https:') !== 0) {

				if ($location.port() === 80) {
					cfgBaseUrl = $location.protocol() + '://' + $location.host() + cfgBaseUrl;
				} else {
					cfgBaseUrl = $location.protocol() + '://' + $location.host() + ':' + $location.port() + cfgBaseUrl;
				}

			}


			if (cfgInterceptJQueryAjaxCalls) {
				// Hack jQuery's AJAX function to take into account the OAuth authentication.
				var jQueryAjaxFn = jQuery.ajax;
				jQuery.ajax = function jqAjaxInterceptor (config) {
					if (shouldSignRequest(config.url)) {
			...
			// (Re)Load OAuth information from the LocalStorage (if any).
			oauthData = localStorageService.get(cfgLocalStorageKeyName);
			if (oauthData) {
	...
	/**
	 * Add Request and Response interceptors to handle OAuth authentication
	 * transparently right into Angular's $http service.
	 */
	app.config(['$httpProvider', function ($httpProvider) {
		...
		$httpProvider.interceptors.push(['$q', '$rootScope', function ($q, $rootScope) {
			...
			return {
				...
				// Request interceptor: signs the request if needed.
	...
	/**
	 *
	 * FUNCTIONS FROM jsOAuth
	 * https://github.com/bytespider/jsOAuth
	 *
	 * /!\ Warning:
	 * Some functions may have been modified a bit.
	 *
	 */
	...
	OAuth.prototype = {
		...
		init: function (options) {
			...
			this.request = function (options) {
				...
				signature = OAuth_signatureMethod[signatureMethod](oauth.consumerSecret, oauth.accessTokenSecret, signatureString);
				...
				if(appendQueryString || method == 'GET') {
					...
					if (typeof data == 'string') {
						...
						for(i in data) {
							query.push(OAuth_urlEncode(i) + '=' + OAuth_urlEncode(data[i] + ''));
						}
		...
		parseTokenRequest: function (tokenRequest, content_type) {
			...
			switch(content_type)
			{
				case "text/xml":
					...
					obj[OAuth_urlDecode(token[0])] = OAuth_urlDecode(secret[0]);
					break;
				...
				default:
					...
					for (; i < len; ++i) {
						...
						obj[OAuth_urlDecode(pair[0])] = OAuth_urlDecode(pair[1]);
					}
			}
	...
	var OAuth_signatureMethod = {
		...
		'HMAC-SHA1': function (consumer_secret, token_secret, signature_base) {
			var passphrase, signature, encode = OAuth_urlEncode;
		...
		for (i in params) {
			if (params[i] && params[i] !== undefined && params[i] !== '') {
				if (i === 'realm') {
					...
					arr.push(i + '="' + OAuth_urlEncode(params[i]+'') + '"');
				}
			}
		}
	...
	function toSignatureBaseString(method, url, header_params, query_params) {
		var arr = [], i, encode = OAuth_urlEncode;

		for (i in header_params) {
			if (header_params[i] !== undefined && header_params[i] !== '') {
				arr.push([OAuth_urlEncode(i), OAuth_urlEncode(header_params[i]+'')]);
			}
		}
	...
	function Collection(obj) {
		var args = arguments, args_callee = arguments.callee, args_length = args.length,
			i, collection = this;
	 ...
	 */
	function URI(url) {
		var args = arguments, args_callee = arguments.callee,
	...
	function QueryString(obj){
		var args = arguments, args_callee = arguments.callee, args_length = args.length,
			i, querystring = this, decode = OAuth_urlDecode;
	...
	QueryString.prototype.setQueryParams = function (query) {
		var args = arguments, args_length = args.length, i, query_array,
			query_array_length, querystring = this, key_value, decode = OAuth_urlDecode;
