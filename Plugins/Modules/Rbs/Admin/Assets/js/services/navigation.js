(function() {
	...
	app.provider('RbsChange.Navigation', function() {
		this.$get = ['$rootScope', '$location', '$timeout', function($rootScope, $location, $timeout) {
			...
			function startSelectionContext(targetUrl, valueKey, params) {
				...
				if (context.hasData()) {
					var url = $location.url(), parts = url.split('#');
					if (parts.length == 2) {
						var fromContext = parts[1];
						if (indexOfContextId(fromContext) != -1) {
							context.fromContext = fromContext;
						}
					}
					context.url = parts[0] + '#' + context.id;
	 ...
	 */
	app.directive('rbsNavigationHistory', [function() {
		return {
			restrict: 'E',
			template:
				'<div data-ng-repeat="c in activeContexts">' +
					'<div class="cascading-forms-collapsed" data-ng-style="getStyle($index)">' +
						'<a href="javascript:;" data-ng-click="c.redirect()"><i class="icon-circle-arrow-left icon-large"></i> (= c.label() =)</a>' +
						'<span data-ng-if="c.labelKey()"> &mdash; <span data-ng-bind="c.labelKey()"></span></span>' +
					'</div>' +
				'</div>',
			scope: {},
