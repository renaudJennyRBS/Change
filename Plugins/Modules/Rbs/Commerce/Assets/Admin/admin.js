(function () {
	...
	function RbsCommerceInitializeReturnProcessCtrl ($scope, $http, REST, i18n, NotificationCenter, ErrorFormatter)
	{
		$scope.data = {websiteId: null, storeId: null, sidebarTemplateId: null, noSidebarTemplateId: null, printTemplateId: null, LCID: null, userAccountTopicId: null};

		$scope.initializeReturnProcessStructure = function () {
			$scope.onInitialization = true;
			$http.post(REST.getBaseUrl('commands/rbs_commerce/initialize-return-process'), {
				websiteId: $scope.data.websiteId,
				storeId: $scope.data.storeId,
				sidebarTemplateId: $scope.data.sidebarTemplateId,
				noSidebarTemplateId: $scope.data.noSidebarTemplateId,
				printTemplateId: $scope.data.printTemplateId,
				LCID: $scope.data.LCID,
				userAccountTopicId: $scope.data.userAccountTopicId
			}).success(function (){
				$scope.onInitialization = false;
				$scope.alreadyInitialized = true;
				NotificationCenter.info(null, i18n.trans('m.rbs.commerce.admin.initialize_return_process_success | ucf'));
			}).error(function (error){
				console.error(error);
				$scope.onInitialization = false;
				NotificationCenter.error(i18n.trans('m.rbs.commerce.admin.initialize_return_process_error | ucf'),
					ErrorFormatter.format(error));
			});
		};
		...
		function checkAlreadyInitialized(websiteId, storeId) {
			$http.post(REST.getBaseUrl('Rbs/Generic/DocumentCodeContextExist'), {
				context: 'Rbs Commerce Return Process Initialize ' + websiteId + ' ' + storeId
			}).success (function (data) {
				$scope.alreadyInitialized = data.result;
				if (data.result) {
					NotificationCenter.warning(null, i18n.trans('m.rbs.commerce.admin.already_initialized_return_process | ucf'));
				}
			}).error (function (error) {
				console.error(error);
				NotificationCenter.error(i18n.trans('m.rbs.generic.admin.check_already_initialized_error | ucf'),
	...
	RbsCommerceInitializeReturnProcessCtrl.$inject = ['$scope', '$http', 'RbsChange.REST', 'RbsChange.i18n', 'RbsChange.NotificationCenter', 'RbsChange.ErrorFormatter'];
	app.controller('RbsCommerceInitializeReturnProcessCtrl', RbsCommerceInitializeReturnProcessCtrl);
})();