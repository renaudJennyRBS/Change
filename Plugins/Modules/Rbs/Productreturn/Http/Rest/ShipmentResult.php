...
/**
 * @name \Rbs\Productreturn\Http\Rest\ShipmentResult
 */
class ShipmentResult
{
	 ...
	 */
	public function productReturnShipmentCollection(\Change\Http\Event $event)
	{
		...
		$query = $event->getApplicationServices()->getDocumentManager()->getNewQuery('Rbs_Productreturn_Shipment');
		$query->andPredicates($query->eq('productReturnId', $productReturnId));
		...
		if ($count && $startIndex < $count)
		{
			...
			{
				/* @var $document \Rbs\Productreturn\Documents\Shipment */
