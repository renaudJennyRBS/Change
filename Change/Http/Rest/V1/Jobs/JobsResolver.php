...
class JobsResolver implements \Change\Http\Rest\V1\NameSpaceDiscoverInterface
{
	...
	{
		$namespaces = [];
		$names = [Job\JobInterface::STATUS_WAITING, Job\JobInterface::STATUS_RUNNING,
	...
	public function resolve($event, $resourceParts, $method)
	{
		...
		elseif ($nbParts == 1 && $method === Request::METHOD_GET)
		{
			if ($event->getParam('isDirectory'))
			{
				...
				$statues = [Job\JobInterface::STATUS_WAITING, Job\JobInterface::STATUS_RUNNING,
					Job\JobInterface::STATUS_SUCCESS, Job\JobInterface::STATUS_FAILED];
	...
	public function runJob(Event $event)
	{
		...
		if ($job)
		{
			...
			else
			{
				...
				$result = new V1\ErrorResult($errorCode, $errorMessage, $httpStatusCode);
