...
/**
 * @name \Change\Http\Rest\V1\ValueConverter
 */
class ValueConverter
{
	 ...
	 * @return array|\Change\Http\Rest\V1\Resources\DocumentLink|null|string
	 * @throws \RuntimeException
	 */
		...
		switch ($type)
		{
			...
			case Property::TYPE_STORAGEURI:
				if (is_string($propertyValue))
				{
					...
					if ($this->urlManager)
					{
						$link = new Link($this->getUrlManager(), V1\Storage\StorageResolver::buildPathInfo($propertyValue));
						$restValue['links'][] = $link->toArray();
						$link = new Link($this->getUrlManager(), V1\Storage\StorageResolver::buildPathInfo($propertyValue), 'data');
