...
class ConjunctionTest extends \PHPUnit_Framework_TestCase
{
	...
	{
		$i = new \Change\Db\Query\Predicates\Conjunction();
		$this->assertInstanceOf('\Change\Db\Query\Predicates\InterfacePredicate', $i);
		
		$i = new \Change\Db\Query\Predicates\Conjunction(new \Change\Db\Query\Expressions\Raw('f1'), new \Change\Db\Query\Expressions\Raw('f2'));
	 ...
	 * @param \Change\Db\Query\Predicates\Conjunction $i
	 */
	public function testArguments(\Change\Db\Query\Predicates\Conjunction $i)
	{	
	 ...
	 * @param \Change\Db\Query\Predicates\Conjunction $i
	 */
	public function testToSQL92String(\Change\Db\Query\Predicates\Conjunction $i)
	{		
		$this->assertEquals('(f1 AND f2)', $i->toSQL92String());
	}
}
